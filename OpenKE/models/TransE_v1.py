...
class TransE(Model):
	...
	def embedding_def(self):
		...
		self.ent_embeddings = tf.get_variable(name = "ent_embeddings", shape = [self.entTotal, config.hidden_size], initializer = tf.keras.initializers.glorot_normal())
		self.rel_embeddings = tf.get_variable(name = "rel_embeddings", shape = [self.relTotal, config.hidden_size], initializer = tf.keras.initializers.glorot_normal())
	...
	def loss_def(self):
		...
		# pos_h, pos_t, pos_r = self.trainModel_pos.get_positive_instance(in_batch = True)
		# neg_h, neg_t, neg_r = self.trainModel_pos.get_negative_instance(in_batch = True)
		...
		sorted_n_scores = tf.argsort(n_score, axis=1, direction='DESCENDING')

		self.preds_h = tf.gather(self.neg_h, sorted_n_scores, batch_dims=1)
		self.preds_t = tf.gather(self.neg_t, sorted_n_scores, batch_dims=1)
		self.preds_r = tf.gather(self.neg_r, sorted_n_scores, batch_dims=1)
