...
class Config(object):
	...
	def __init__(self):
		...
		self.samples_contrastive_learning = 1000
	...
	def generate_neg_samples_pos_model(self, all_heads, all_tails, all_relations):
		...
		if self.samples_contrastive_learning < len(unique_heads):
			selected_heads_list = [np.random.choice(unique_heads, size=self.samples_contrastive_learning, replace=False) for _ in range(len(all_tails))]
			test_t_perturb_heads = np.repeat(all_tails, self.samples_contrastive_learning)  # Repeat heads for sampled tails
			test_r_perturb_heads = np.repeat(all_relations, self.samples_contrastive_learning)  # Repeat relations accordingly
		else:
		...
		# Randomly select 100 unique tails for each head-relation pair
		if self.samples_contrastive_learning < len(unique_tails):
			selected_tails_list = [np.random.choice(unique_tails, size=self.samples_contrastive_learning, replace=False) for _ in range(len(all_heads))]
			test_h_perturb_tails = np.repeat(all_heads, self.samples_contrastive_learning)  # Repeat heads for sampled tails
			test_r_perturb_tails = np.repeat(all_relations, self.samples_contrastive_learning)
		else:
		...
		if self.samples_contrastive_learning < len(unique_heads):
			res_perturb_heads = res_perturb_heads.reshape(len(all_heads), self.samples_contrastive_learning)
		else:
			res_perturb_heads = res_perturb_heads.reshape(len(all_heads), len(unique_heads))

		if self.samples_contrastive_learning < len(unique_tails):
			res_perturb_tails = res_perturb_tails.reshape(len(all_tails), self.samples_contrastive_learning)
		else:
