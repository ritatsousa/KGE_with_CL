...
void importTrainFiles() {
	...
	printf("The file is train2id_pos.txt.\n");
	FILE *fin;
	int tmp;
	...
	fin = fopen((inPath + "train2id_pos.txt").c_str(), "r");

	if (fin == nullptr) {
		std::cout << '`' << inPath << "train2id_pos.txt" << '`' << " does not exist"
		          << std::endl;
		return;
	}
...
void importTestFiles() {
	...
	FILE *f_kb1 = fopen((inPath + "test2id_pos.txt").c_str(), "r");

	if (f_kb1 == nullptr) {
		std::cout << '`' << inPath << "test2id_pos.txt" << '`' << " does not exist"
		          << std::endl;
		return;
	}

	FILE *f_kb2 = fopen((inPath + "train2id_pos.txt").c_str(), "r");
	if (f_kb2 == nullptr) {
		std::cout << '`' << inPath << "train2id_pos.txt" << '`' << " does not exist"
		          << std::endl;
		return;
	...
	FILE *f_kb3 = fopen((inPath + "valid2id_pos.txt").c_str(), "r");
	if (f_kb3 == nullptr) {
		std::cout << '`' << inPath << "valid2id_pos.txt" << '`' << " does not exist"
		          << std::endl;
		return;
	}
